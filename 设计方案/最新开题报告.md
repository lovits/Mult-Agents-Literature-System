...
3. 使用 Structure Segmenter 按标题、段落、表格、图注、算法、公式、页边界和附录切分文档块。
4. 使用 Section Canonicalizer 将章节别名映射到统一标签空间。该模块以规则词典为主，
   LLM 只在低置信别名或语义标签映射中辅助判断，并且只能从固定标签集合中选择。
5. 为每个文档块保存原始章节名、统一章节标签、段落位置、页码、证据类型、解析器和解析置信度。
