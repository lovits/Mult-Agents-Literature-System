...
1. 将 PDF、Markdown、OpenReview JSON 和普通 JSON 统一转换为 Markdown 中间表示。
2. 使用 Source Adapter 记录来源、checksum 和元数据，使用 Document Normalizer 生成 `paper.md`
   与 `parse_manifest.json`。
3. 使用 Document Parsing Agent 按标题、段落、表格、图注、算法、公式、页边界和附录切分文档块，
   并由 LLM 对章节别名和证据类型进行主路径标签映射。
4. Document Parsing Agent 只能从固定章节标签和固定证据类型集合中选择；代码层只负责 JSON schema
   校验、固定标签集合约束、低置信 fallback 和解析轨迹记录，不设置规则词典优先路径。
5. 为每个文档块保存原始章节名、统一章节标签、段落位置、页码、证据类型、解析器、LLM 置信度、
   prompt 版本和 fallback 原因。
6. 使用 BM25 与 Dense Retrieval 分别进行关键词和语义检索。
7. 使用 Reciprocal Rank Fusion 合并检索结果。
8. 根据目标章节和期望证据类型施加先验。
9. 对高排名段落进行邻接扩展，保留上下文。
10. 返回带来源位置的证据包。
