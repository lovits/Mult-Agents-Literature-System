...
Markdown 中间表示，再进行结构切块、章节标签归一和证据块映射：
...
负责从 Markdown 中识别 heading、paragraph、table、figure caption、algorithm、equation、appendix、
reference 和 page boundary。切块不依赖固定章节字段名，而根据 Markdown 结构和布局标记生成
`DocumentBlock`。

### Section Canonicalizer

负责把 `Technical Overview`、`Approach`、`Preliminary`、`Experimental Evaluation`、
...
Canonicalizer 采用“规则优先、LLM 辅助、固定标签约束”的设计：

1. 先使用可版本化 alias 词典进行确定性映射；
2. 只有当规则无法判断或置信度低时，才调用 LLM 判断章节标签；
3. LLM 只能从固定标签集合中选择，不能创造新标签；
4. LLM 必须输出 `canonical_section`、`confidence`、`rationale` 和 `matched_cues`；
5. 代码层对 LLM 输出做 JSON schema 校验，非法标签退回 `unknown`；
6. 原始章节标题必须保存在 metadata 中，不能被覆盖。

LLM 的作用是处理别名和语义标签对齐，不负责改写正文、不负责直接生成证据结论。这样既利用
LLM 对复杂标题的语义理解能力，又保持 Paper-RAG 输入空间稳定、可消融、可复现。
...
Stage 0 论文解析
  ...
  -> Structure Segmenter
  -> Rule + LLM Section Canonicalizer
  -> EvidenceBlock
  -> Paper Evidence Index
