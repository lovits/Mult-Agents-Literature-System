...
- Create: `实验/src/evireview/service/structure_segmenter.py`
- Create: `实验/src/evireview/service/section_canonicalizer.py`
...
- Create: `实验/tests/unit/test_section_canonicalizer.py`
...
- [ ] **Step 3: 编写 LLM 章节标签映射测试**

LLM 只作为低置信章节别名映射器，不允许生成任意标签。测试中用 fake LLM provider，避免外部调用。

```python
from evireview.service.section_canonicalizer import SectionCanonicalizer

def test_llm_section_mapping_is_constrained_to_fixed_labels():
    canonicalizer = SectionCanonicalizer(llm_provider=FakeSectionLLM("method"))
    result = canonicalizer.canonicalize("Technical Overview", text_hint="We define the model.")
    assert result.canonical_section == "method"
    assert result.source == "llm"
    assert result.confidence >= 0.7

def test_invalid_llm_section_mapping_falls_back_to_unknown():
    canonicalizer = SectionCanonicalizer(llm_provider=FakeSectionLLM("new invented label"))
    result = canonicalizer.canonicalize("Technical Overview", text_hint="We define the model.")
...
- [ ] **Step 6: 实现 Structure Segmenter**

从 Markdown 中识别 heading、paragraph、table、figure caption、algorithm、equation、appendix、
reference 和 page boundary，输出 `DocumentBlock[]`。长段落按句子边界切块，表格、图注、
算法和公式单独成块。

- [ ] **Step 7: 实现 Rule + LLM Section Canonicalizer**
...
规则词典优先处理常见别名。LLM 只在规则无法判断或低置信时调用，并且只能返回固定标签集合。
LLM 输出必须经过 schema 校验，非法标签回退为 `unknown`。原始章节名保存在 metadata。
...
`parse_confidence`、`page_start/page_end`、`parser`、`canonicalizer_source` 等 metadata。
`PaperIngestionService` 只负责编排 Source Adapter、Normalizer、Segmenter、Canonicalizer 和 Mapper。
...
Run: `python -m pytest tests/unit/test_section_canonicalizer.py -q`
Expected: PASS；规则映射、LLM 映射、非法标签回退均通过。

- [ ] **Step 10: 提交**

```bash
git add 实验/src/evireview/models 实验/src/evireview/dao 实验/src/evireview/service 实验/tests
