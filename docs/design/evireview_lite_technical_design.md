...
14. GLM-4.6V structured reviewer sample：通过环境变量安全接入 GLM-4.6V，请求 10 篇本地样本，最终 10 篇成功生成 27 条结构化弱点；coverage proxy 在 threshold=0.18 时覆盖 59.52% 人工 reviewer weaknesses，并立即进入 section-aware retrieval 与 heuristic verifier；当前标签分布为 9 条 Mentioned but Not Problem、13 条 Partially Supported、2 条 Supported、3 条 Unsupported，说明 provider 接入和流程闭环可用，但仍应报告为 clean 10-paper diagnostic。
15. Generated reviewer paired comparison：在 GLM overlap 的 10 篇论文 / 289 条 human weaknesses 上，GLM-4.6V 的 coverage recall@0.18 为 0.5952，高于同批 rubric-agent 的 0.3875；GLM mean support score 为 0.4670，高于 rubric-agent 的 0.2000。该结果支持 GLM/LLM reviewer 作为候选生成器，rubric-agent 作为可解释结构风险 baseline，但不能写成最终 provider 排名。
16. Hierarchical Paper-RAG retrieval tools：新增 keyword_search、semantic_search、section_read 和 RRF merge，对已有 generated weaknesses 重新检索证据。GLM 样本的 mean support score 从 0.4670 提升到 0.5212，Partially Supported-or-better rate 从 0.5556 提升到 0.6667；rubric-agent 提升有限，说明 hierarchical retrieval 更适合具体 weakness，而不是泛化结构风险提示。该结果仍是 silver verifier 诊断，后续需用人工 gold labels 验证。
17. Generated weakness evidence-aware ranker：新增 generated_weakness_evidence_aware_ranker_v0，用 `(0.45 + support_score) * verifier_label_weight * severity_weight` 对 generated weaknesses 做每篇 top-3 排序。GLM top-3 保持 mean support=0.5212、partial+=0.6667；rubric-agent top-3 mean support 从候选均值 0.1999 提升到 0.2343，说明 ranker 可把 evidence/verifier 结果转成可审计弱点 shortlist，但仍是 silver-label diagnostic。
18. Human weakness hierarchical retrieval：对 1463 条人工 reviewer weaknesses 运行 hierarchical Paper-RAG，non-empty retrieval=1.0、Top-1 section alignment=0.9993、Top-3 alignment=1.0；section-aware 与 hierarchical 的 Top-1 disagreement=0.6138、Top-3 disagreement=0.9645，并已生成 300 条 retrieval comparison annotation queue。该队列是下一步人工 gold 检索对比的主入口。
19. Retrieval comparison gold scaffold：已实现 comparison gold 导入和 evaluation 脚本；当前 gold rows=0、status=needs_labels，因此只说明管线就绪，不说明 retriever 最终胜负。
20. OpenRouter chat reranker/verifier 诊断：免费 chat reranker 当前受 429 限速影响；embedding max-similarity verifier 的 pilot-selected main Macro-F1 仍为 0.4106，说明 embedding 适合 retrieval，但不能单独承担 verifier。
...
4. OpenRouter 免费 chat reranker 受上游 429 限速影响，暂不作为全量主线；GLM-4.6V 当前作为 structured reviewer provider 已完成 clean 10-paper 小样本对比，后续冻结为实验章节诊断表并继续回到 verifier/gold-label 缺口。
