...
14. GLM-4.6V structured reviewer sample：通过环境变量安全接入 GLM-4.6V，请求 10 篇本地样本，其中 8 篇成功生成 21 条结构化弱点，2 篇因 JSONDecodeError 失败；coverage proxy 在 threshold=0.18 时覆盖 56.77% 人工 reviewer weaknesses，并立即进入 section-aware retrieval 与 heuristic verifier；当前标签分布为 6 条 Mentioned but Not Problem、10 条 Partially Supported、2 条 Supported、3 条 Unsupported，说明 provider 接入和流程闭环可用，但仍应报告为 8-paper diagnostic。
15. Generated reviewer paired comparison：在 GLM overlap 的 8 篇论文 / 229 条 human weaknesses 上，GLM-4.6V 的 coverage recall@0.18 为 0.5677，高于同批 rubric-agent 的 0.4148；GLM mean support score 为 0.4744，高于 rubric-agent 的 0.2079。该结果支持 GLM/LLM reviewer 作为候选生成器，rubric-agent 作为可解释结构风险 baseline，但不能写成最终 provider 排名。
16. Hierarchical Paper-RAG retrieval tools：新增 keyword_search、semantic_search、section_read 和 RRF merge，对已有 generated weaknesses 重新检索证据。GLM 样本的 mean support score 从 0.4744 提升到 0.5304，Partially Supported-or-better rate 从 0.5714 提升到 0.7143；rubric-agent 提升有限，说明 hierarchical retrieval 更适合具体 weakness，而不是泛化结构风险提示。该结果仍是 silver verifier 诊断，后续需用人工 gold labels 验证。
17. Generated weakness evidence-aware ranker：新增 generated_weakness_evidence_aware_ranker_v0，用 `(0.45 + support_score) * verifier_label_weight * severity_weight` 对 generated weaknesses 做每篇 top-3 排序。GLM top-3 保持 mean support=0.5304、partial+=0.7143；rubric-agent top-3 mean support 从候选均值 0.1999 提升到 0.2343，说明 ranker 可把 evidence/verifier 结果转成可审计弱点 shortlist，但仍是 silver-label diagnostic。
...
4. OpenRouter 免费 chat reranker 受上游 429 限速影响，暂不作为全量主线；GLM-4.6V 当前作为 structured reviewer provider 已完成 8 篇有效小样本对比，后续优先修复 2 篇 JSON 解析失败或回退到本地人工 gold label 流程。
