...
执行结果：E6 报告组装阶段已完成六步。第一步使用 OpenReview seed 的 10 篇论文
和 41 条 Official Review 打通 report assembly，再将 OpenReview seed 扩大到
30 篇论文、122 条 Official Review，且 30 个 PDF 均通过本地有效性检查。第二步
从官方评审 weakness 字段抽取候选弱点并生成 B1 review-derived Top-K 结构化报告，
用作可追踪报告上界。第三步新增 B2 system-generated deterministic baseline，只读取
论文标题、摘要、关键词和领域元数据，不读取 Official Review。相对 B0
Unstructured Review Dump，B2 的 Trace Coverage 从 0.0000 提升到 1.0000，
Top-K Compliance 为 1.0000，Paper Report Coverage 为 1.0000，Review Leakage
Free 为 true，Accept/Reject Decision 数量为 0。在 30 篇扩展 seed 上，B2 与
Official Review weakness 的弱文本重叠 proxy 为 0.0504，说明本地启发式候选生成
可以打通自动评审链路，但候选质量仍较弱，不能宣称已经达到高质量自动评审。
第四步新增 B3 cue-aware deterministic baseline，在相同无泄漏输入边界下根据
title/abstract/keywords 中的任务线索选择候选模板。B3 的 Trace Coverage、
Top-K Compliance、Paper Report Coverage 均为 1.0000，Review Leakage Free 为
true，Accept/Reject Decision 数量为 0；Official Weakness Proxy Overlap@K 从 B2
的 0.0504 提升至 0.0549，delta 为 0.0044，Aspect Diversity@K 为 1.0000，
Redundancy Rate@K 为 0.0000。该优化方向在更大 seed 上仍然成立，但幅度缩小，
只能说明 cue-aware 模板带来稳定的小幅改进。arXiv unseen 5 篇论文只生成 demo
manifest，不报告 Gold 指标。第五步新增 E6 candidate diagnostics，使用 Official
Review weakness 只做诊断性 proxy，不进入候选生成。诊断结果显示 B3 在 30 篇中
19 篇优于 B2、11 篇退化，failure-or-tie rate 为 0.3667；B3 zero-overlap rate
从 B2 的 0.0556 降至 0.0222。按 aspect 看，B3 的 method、missing_baseline 和
related_work overlap 较好，但 experiment aspect 仍是最低项之一。该诊断不扩大
B3 的效果声明，只用于确定下一步 provider-generated candidates 的失败样本切片。
第六步新增 E6-P DeepSeek provider-generated candidates 对照，在 E6-D 的 8 个
failure cases 上调用 `deepseek-v4-flash-free`，prompt 输入只包含论文元数据和
本地候选上下文，不包含 Official Review。该实验完成但成功标准未达成：
Provider 输出覆盖率为 0.0000，provider_failures 为 8，P1 Proxy Overlap@K 为
