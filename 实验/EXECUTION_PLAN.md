...
1. 建立 E6 端到端报告生成输入 manifest。
2. 串联 E2/E3/E4/E5 的已冻结输出，生成结构化审计轨迹。
3. 在 OpenReview seed 上输出 Top-K 自动评审报告样例。
4. 在 arXiv unseen 上做未见论文演示，只报告运行完整性、证据引用和成本，不报告 Gold 指标。
